...
int table_check_key_ks1(Table *table, char *key1){
	for (int i=0; i<table->msize1; i++){
...
void table_add_ks1_static(Table *table, Item *item, char *key1){
	if (table->csize1 < table->msize1){
		for (int i=0; i<table->msize1; i++){
			if (table->ks1[i].busy!=1){
				memcpy(table->ks1[i].key, key1, STRLEN*sizeof(char));
				//table->ks1[i].key = key;
...
void table_reorganize_ks1(Table *table){
	...
	while (count!=table->csize1){
		...
		table->ks1[i].busy = 1;
		table->ks1[j].busy = 0;
		memcpy(table->ks1[i].key, table->ks1[j].key, STRLEN*sizeof(char));
